...
namespace inmobiliaria_front.Pages.Ventanas
{
    ...
    public class ContratosContadosModel : PageModel
    {
        ...
        public List<ContratosContados>? contratos { get; set; }
        public List<ContratosContados>? contratosPendientes { get; set; }
        public List<ContratosContados>? contratosAceptados { get; set; }
        public List<ContratosContados>? contratosSolicitudes { get; set; }
        public List<ContratosContados>? contratosNoAceptados { get; set; }
        ...
        public ContratosContadosModel()
        {
            IContratosContadosnegocio = new ContratosContadoNegocio();
        ...
        public void OnPostBtRefrescar()
        {
            ...
            {
                cargarlistas();
                if (IContratosContadosnegocio == null)
                    return; ;
                Contrato = null;
                Aceptar = false;
            }
        ...
        {
            cargarlistas();
            Contrato = new ContratosContados()
            {
        ...
        public void OnPostBtGuardar()
        {
            try
            {
                ...
                if (Contrato.Id == 0)
                    Contrato = IContratosContadosnegocio!.Guardar(Contrato!);
                else
                    Contrato = IContratosContadosnegocio!.Modificar(Contrato!);
        ...
        public void OnPostBtBorrar()
        {
            try
            {
                if (Contrato == null)
                    return;
                ViewData["Mensaje"] = IContratosContadosnegocio!.Eliminar(Contrato!);
                Contrato = null;
            }
            catch (Exception ex)
            {
        ...
        public void OnPostBtAceptarContrato()
        {
            try
            {
                contratosSolicitudes = IContratosContadosnegocio!.Consultar().Where(c => c.Estado == "Solicitud").ToList();
                ...
                Contrato = IContratosContadosnegocio!.Modificar(contra!);
                OnPostBtCerrar();
            }
            catch (Exception ex)
            {
